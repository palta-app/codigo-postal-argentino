...
async function streetNumberExtract (link, streetId, streetName) {
    try{
        ...
        $patern.each((idx, item)=>{
            streetList.push({
                streetId,
                streetName,
                from: $(item).find('tr td:nth-child(2)').text(),
                until: $(item).find('tr td:nth-child(3)').text(),
                odd: $(item).find('tr td:nth-child(4)').text(),
                cpa:  $(item).find('tr td:nth-child(6)').text(),
            })
        })
    ...
    module.exports = streetNumberExtract;