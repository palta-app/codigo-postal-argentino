...
async function locationExtract(name, link, startId) {
    try{
        ...
        const $locations = $('.cities a');
        let id = startId
        $locations.each((idx, item)=>{
            id++;
            const locationName = $(item).text();
            const linkToLocation = $(item).attr('href');
            arrayLocation.push({
                locationId: id,
                locationName,
                locationLink: linkToLocation
            })
        })
        return {
...
module.exports = locationExtract;