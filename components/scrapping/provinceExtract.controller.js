...
async function provinceExtract (){
    try{
        ...
        const {data} = await axios.get("https://codigo-postal.co/argentina/", { headers: {
                                                 'User-Agent': headChoosen }  });
        const $ = cheerio.load(data);
        
        const arrayProvinces = [];
        const arrayLinksToProvinces = [];
        
        const $province = $('.column-list li');
        $province.each((idx, item)=>{
            const provinceName = $(item).text();
            arrayProvinces.push(provinceName);
        })
        
        const $linkProvince = $('.column-list a');
        $linkProvince.each((idx, item)=>{
            const linkToProvince = $(item).attr('href');
            arrayLinksToProvinces.push(linkToProvince);
        })
        
        const provincesLevel = [];
        for(let x=0; x<arrayProvinces.length;x++) {
            provincesLevel.push({
                provinceName: arrayProvinces[x],
                provinceLink: arrayLinksToProvinces[x]
            })
        }
...
module.exports =  provinceExtract
