...
async function getResultsFromCPA(query) {
    ...
    const listResult = await page.evaluate(() => {
     ...
     await internalPage.waitForSelector('ul[class="column-list"]'); // Reemplaza 'selector-de-elemento' con el selector correspondiente
 
     // Extraer información de la página interna utilizando page.evaluate
     const internalData = await page.evaluate(() => {
        let result = [];
        document.querySelectorA('div[class="col-md-6"] ul li').forEach((li, i) => {
            const a = li.querySelector('a');
            result.push({
                index: i,
                title: a.innerText,
