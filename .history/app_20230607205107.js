...
async function getResultsFromCPA(query) {
  ...
  const selectedInternalUrlLocalities = listResultProvince[0].url;
  const nextPageLocalities = await browser.newPage();
  await nextPageLocalities.goto(selectedInternalUrl);
  await nextPageLocalities.waitForSelector('ul[class="cities"]');

  const listResultLocalitiesDos = await nextPage.evaluate(() => {
    let result = [];
    document.querySelectorAll('ul[class="cities"] li').forEach((li, i) => {
      const a = li.querySelector("a");
      result.push({
