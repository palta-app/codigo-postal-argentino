...
async function getResultsFromCPA(query) {
  ...
  for (const selectedUrl of selectedUrls) {
    ...
    const listResultProvince = await internalPage.evaluate(() => {
      let result = [];
      document
        .querySelectorAll('ul[class="column-list"] li')
        .forEach((li, i) => {
    ...
    for (const selectedInternalUrl of selectedInternalUrls) {
      ...
      const selectedInternalUrlsLocalities = listResultLocalities.map(
        (result) => result.url
      );
      for (const selectedInternalUrlLocalities of selectedInternalUrlsLocalities) {
        ...
        await nextPageLocalities.waitForSelector(
        ...
        const selectedInternalUrlsStreet = listResultLocalitiesDos.map(
          (result) => result.cpaUrl
        );
        for (const selectedInternalUrlStreet of selectedInternalUrlsStreet) {
          ...
          const selectedInternalUrlsEndStreet = listResultStreet.map(
            (result) => result.url
          );
          for (const selectedInternalUrlEndStreet of selectedInternalUrlsEndStreet) {
            ...
            await nextPageEndStreet.waitForSelector(
              "div.table-responsive table tbody"
            );
            ...
            const selectedInternalUrlsHeight = listResultEndStreet.map(
              (result) => result.cpaUrl
            );
            for (const selectedInternalUrlHeight of selectedInternalUrlsHeight) {
              ...
              await nextPageHeight.waitForSelector(
                "div.table-responsive table tbody"
              );
